...
const LabModal: React.FC<LabModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-deep-black/80 backdrop-blur-sm">
      <div className="bg-stark-white w-full max-w-4xl max-h-[90vh] overflow-y-auto brutalist-border brutalist-shadow-yellow p-8 md:p-12 relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 bg-deep-black text-white p-2 hover:bg-brand-pink transition-colors brutalist-border-sm"
        >
          <span className="material-symbols-outlined">close</span>
        </button>

        <h2 className="text-4xl md:text-6xl font-display text-deep-black uppercase tracking-tighter mb-8">
          AI DESIGN <span className="text-brand-blue">LAB V2</span>
        </h2>
        ...
        <div className="mb-10">
          ...
          <textarea
            ...
            className="w-full h-32 brutalist-border p-6 text-xl font-body font-bold focus:ring-0 focus:border-brand-yellow transition-all"
          />
          <button
            onClick={handleGenerate}
            disabled={loading}
            className="mt-6 bg-brand-yellow text-deep-black w-full py-6 text-2xl font-display uppercase brutalist-border brutalist-shadow hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all disabled:opacity-50"
          >
        ...
        {result && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-brand-blue text-white p-8 brutalist-border brutalist-shadow-sm">
              <h3 className="text-2xl font-display uppercase mb-4 text-brand-yellow">Strategic Angle</h3>
            ...
            <div className="bg-brand-pink text-white p-8 brutalist-border brutalist-shadow-sm">
